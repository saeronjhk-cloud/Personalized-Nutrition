  ...
  function renderReportForm(kind: 'new' | 'existing') {
      ...
      return (
        <div>
          ...
          <button
            ...
            style={{
              background: 'none', border: 'none', padding: 'var(--space-3) 0 0', cursor: 'pointer',
    ...
    return (
      <div>
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: 12, marginTop: 'var(--space-2)', lineHeight: 1.5 }}>
          {analysis
    ...
    <div className="survey-container fade-in">
      <div className="survey-card" style={{ marginBottom: 'var(--space-4)' }}>
        ...
        <form onSubmit={onSubmit} style={{ display: 'flex', gap: 'var(--space-2)' }}>
          ...
          <button type="submit" className="btn btn-secondary" style={{ width: 'auto', padding: 'var(--space-3) var(--space-5)' }} disabled={loading}>
            {loading ? '조회 중…' : '검색'}
          </button>
        </form>
            ...
            {history.slice(0, 5).map((r) => (
              <li key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
                ...
                <button type="button" className="btn btn-secondary" aria-label="이력 삭제"
                  style={{ width: 'auto', padding: 'var(--space-2) var(--space-3)', color: 'var(--text-muted)', flexShrink: 0 }}
      ...
      {result && (
        <>
            ...
            <div className="survey-card" style={{ marginBottom: 'var(--space-4)' }}>
              {reportOpen || confirmed ? (
                ...
                <>
                  ...
                  <p style={{ color: 'var(--text-secondary)', fontSize: 13, lineHeight: 1.6, marginBottom: 'var(--space-3)' }}>
                    {GAP_HELP_TEXT}
                  </p>
                  <div style={{ display: 'flex', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
          ...
          {personal && personal.applicable ? (
            ...
            <div className="survey-card" style={{ marginBottom: 'var(--space-4)', position: 'relative', overflow: 'hidden' }}>
              <h3 className="survey-step-title" style={{ fontSize: 16 }}>내 기준으로 보기</h3>
...
function WeekBars({ weeks }: { weeks: ScanSummary['weeks'] }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 'var(--space-1)', height: 32 }} aria-hidden="true">
      {weeks.map((w) => (
