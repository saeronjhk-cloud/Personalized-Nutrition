      ...
      <div className="card" style={{ lineHeight: 1.8, fontSize: 14, color: 'var(--text-secondary)' }}>
        ...
        {[
          ...
          ...(MEAL_ENABLED ? [
            ...
            {
              name: 'OpenAI, L.L.C. (OpenAI OpCo, LLC)', country: '미국',
              ...
              keep: 'OpenAI는 요청 처리·오남용 방지를 위해 입력·출력을 최대 30일간 보관할 수 있으며(관계 법령 의무 이행 또는 중대한 안전위험 대응 시 예외적으로 더 길 수 있음), API로 전송된 데이터를 기본적으로 모델 학습에 사용하지 않습니다. 회사는 데이터 공유(모델 개선) 기능을 활성화하지 않습니다.',
          ...
          <div key={v.name} className="card" style={{ padding: '12px 14px', marginBottom: 12, fontSize: 13, lineHeight: 1.7 }}>
            ...
            <div><strong>법적 근거:</strong> 「개인정보 보호법」 제28조의8 제1항 제3호(계약의 이행에 필요한 처리위탁·보관으로서 본 처리방침을 통해 공개)</div>
          </div>
        ))}

        {MEAL_ENABLED && (
          <p style={{ marginBottom: 16, fontSize: 13, color: 'var(--text-muted)' }}>
            ⓘ <strong>식사 사진의 최소 전송</strong> — 식사 사진 분석에는 AI 처리자(OpenAI)가 이용됩니다. 회사는 이용자 사진의 국외 이전을
            최소화하기 위해, 전체 원본 사진이 아니라 <strong>음식으로 판단된 영역만 잘라 저해상도로 축소한 이미지</strong>만 전송하며 원본 사진은 전송하지 않습니다.
            전송 전 위치정보 등 부가정보(EXIF)를 제거하고, 회원번호·이메일·전화번호 등 직접 식별정보는 함께 전송하지 않습니다.
            음식 영역 분리가 실패하거나 국외이전 동의가 확인되지 않으면 전송하지 않습니다.
          </p>
        )}
        <p style={{ marginBottom: 16 }}>
        ...
        {MEAL_ENABLED && (
          <p style={{ marginBottom: 16, fontSize: 13, color: 'var(--text-muted)' }}>
            ⓘ <strong>식사 사진 기능의 국외이전 거부·철회</strong> — 식사 사진 분석을 위한 국외이전은 <strong>해당 기능에 한하여</strong> 별도로
            거부·철회할 수 있습니다. 식사 사진 기록 진입 시 '동의하지 않음'을 선택하거나, 이미 동의한 경우 <strong>설정 &gt; 식사 사진 분석 동의 철회</strong>에서
            철회할 수 있습니다. 철회 시 즉시 이후 식사 사진 분석(국외이전)이 중단되며, <strong>계정 및 그 밖의 서비스는 계속 이용</strong>할 수 있습니다.
            이미 전송된 정보는 위 OpenAI 보유·이용 기간에 따라 처리됩니다.
          </p>
        )}
        <p style={{ marginBottom: 16, fontSize: 13, color: 'var(--text-muted)' }}>
        ...
        <p style={{ marginBottom: 16 }}>
          ...
          {MEAL_ENABLED && ' 특히 식사 사진 분석 기능은 이용 전 만 14세 이상임을 확인받으며, 회사 서버가 연령 확인 여부를 재검증하여 만 14세 미만의 민감정보 처리·국외이전을 차단합니다.'}
        </p>
        ...
        <p style={{ marginBottom: 8 }}>
          ...
          {MEAL_ENABLED && ' 식사 사진의 민감정보 처리·국외이전(제4조 Railway·OpenAI) 등 중요한 변경 시에는 기존 이용자에게도 다음 이용 시점에 변경된 내용을 다시 고지하고 재동의를 받은 뒤에만 해당 처리를 계속합니다.'}
        </p>
        {MEAL_ENABLED && (
          <>
