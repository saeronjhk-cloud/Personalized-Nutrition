    ...
    <div className="page fade-in" style={{ paddingTop: 32, paddingBottom: 60 }}>
      ...
      <div className="card" style={{ lineHeight: 1.8, fontSize: 14, color: 'var(--text-secondary)' }}>
        ...
        <ul style={{ marginBottom: 16, paddingLeft: 20 }}>
          ...
          <li><strong>백업 데이터:</strong> 장애 복구를 위해 약 7일간(일 단위 자동 백업) 보관 후 순차적으로 파기합니다. 저장소(Storage)에 보관된 사진 파일은 데이터베이스 백업에 포함되지 않습니다.</li>
          {MEAL_ENABLED && (
        ...
        {[
          {
            name: 'Supabase, Inc.', country: '대한민국(서울 리전 저장) · 운영주체 Supabase, Inc.는 미국 소재',
          ...
          ...(MEAL_ENABLED ? [
            {
              ...
              purpose: '사진 기반 음식·영양 분석 연산(무상태 처리 — 사진 파일을 디스크에 저장하지 않으며, 회원번호·이메일 등 식별정보 대신 임의의 요청 식별자로만 처리)',
        ...
        {MEAL_ENABLED && (
          <p style={{ marginBottom: 16, fontSize: 13, color: 'var(--text-muted)' }}>
            ...
            분석 결과는 지체 없이 삭제됩니다.</strong> <strong>계정 및 그 밖의 서비스는 계속 이용</strong>할 수 있습니다. 백업본에 남아 있는 정보는 복구 목적 외에는 이용되지 않으며, 정해진 백업 보유주기(약 7일)가 지나면 자동 파기됩니다.
        ...
        <ul style={{ marginBottom: 16, paddingLeft: 20 }}>
          ...
          <li>장애 복구용 백업 데이터는 약 7일간(일 단위 자동 백업) 보관 후 순차적으로 파기합니다.</li>
        </ul>
